...
mod.names<-c("depth","no.depth")
aictab(cand.set=models,modnames=mod.names)
```
Select the models based on AIC performance. The no depth linear model performed significantly better based on the $\Delta AIC$ greater than 2. 
...
final_model<-lm(o2sat~t_deg_c+salinity+po4u_m,data=seawater_raw)
```
